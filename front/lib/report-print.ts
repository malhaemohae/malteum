...
import { displayField, displayValue, INTERNAL_FIELDS } from './workspace-model';
...
export function reportHtml(report: ApiReport) {
  ...
  const record = (row: Record<string, unknown>) => `<dl>${Object.entries(row).filter(([key, value]) => value != null && !INTERNAL_FIELDS.includes(key)).map(([key, value]) => `<dt>${escapeHtml(displayField(key))}</dt><dd>${escapeHtml(displayValue(value, key))}</dd>`).join('')}</dl>`;
