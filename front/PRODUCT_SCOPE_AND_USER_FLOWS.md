...
- 랜딩 화면에서 서비스 가치를 이해하고 상담 유형·실행 모드·가상 고객 라벨을 준비한 뒤 브리핑으로 이동한다.
- `예적금 중도해지`와 `주택담보대출` 두 제품 시나리오를 전환한다.
...
- 튜토리얼은 랜딩과 브리핑에서는 열리지 않고, 상담 화면에 진입한 뒤 대시보드 헤더부터 8단계로 안내한다.
...
| `Landing` / `MarketingLanding` | 강한 첫 화면의 서비스 컨셉 소개, 가치·작동 방식·워크스페이스 설명, 접힌 시연 설정, 브리핑 진입 |
| `BriefingScreen` | 세션 전 필수 안내·주의 표현·최근 기준 확인 |
...
| `product` | `deposit` | 세션 설정 제품 버튼 | 제품 설정·이벤트·개입·체크리스트 변경 |
| `mode` | `replay` | 세션 설정 모드 버튼 | 표시 라벨·상태 문구·시간·진행률 변경 |
| `customerLabel` | `가상 고객 A` | 입력 또는 제품 변경 | 대시보드 요약에 표시 |
| `customerType` | `general` | 세션 설정의 고객 유형 | `customer_profile.type`으로 서버에 전달 |
...
| 세션 설정 | 하단 접힌 `시연 설정` 안에서 상담 유형, 입력 모드, 가상 고객 라벨, 일반·전문 금융소비자 유형 선택 |
...
| `mortgage` | 주택담보대출 | 가상 고객 B | 4분 |

제품을 바꾸면 고객 라벨도 각각의 기본값으로 돌아간다. 사용자가 라벨을 직접 입력한 뒤 제품을 바꾸면 해당 제품의 기본 라벨로 교체된다.
...
| `live` | LIVE | 브리핑 후 자동 실시간 녹음 | 대시보드 ready→마이크 입력→STT→판정 |
| `text` | TEXT | 음성 없이 검토 | 직접 입력한 발화를 동일 판정 경로로 전달 |
| `trace` | TRACE | 저장 이벤트를 순서대로 재생 | 이력 세션을 로컬 데모 이벤트로 재생 |

현재의 `고객 라벨`은 개인정보가 아닌 데모용 별칭이다. 실제 연동에서는 화면 라벨과 별개로 `customer_profile.type`(`general`/`professional`)과 허용된 태그를 세션 시작 메시지에 전달해 설명의무 범위를 결정한다.

### 4.4 랜딩 상호작용

| 사용자 행동 | 현재 결과 |
| --- | --- |
| 상단·히어로·최종 `상담 시작하기` 및 `상담 화면 열기` 클릭 | `screen=briefing`으로 변경하고 선택 제품·모드·고객 라벨을 브리핑에 전달. LIVE는 브리핑 확인 후 대시보드의 `녹음 시작` 버튼으로 녹음 on/off |
| `왜 말틈인가`·`작동 방식`·`대시보드에서 보기` 클릭 | 롱폼 랜딩 안에서 해당 설명 섹션으로 이동 |
| 제품 선택 | 제품 설정·고객 기본 라벨 변경 |
| 모드 선택 | 모드 설정과 랜딩/대시보드 상태 문구 변경 |
| 고객 라벨 입력 | 로컬 문자열이 대시보드 요약과 서버 세션 프로필에 전달 |
| 고객 유형 선택 | `customer_profile.type`을 `hello`/세션 생성 요청에 전달 |
| 세션 설정 접기/펼치기 | HTML `details` 요소의 로컬 동작 |
...
- 선택 제품의 pack version(예적금 `DEP-2026.08-v4`, 주담대 로컬 데모 `MTG-2026.08-v1`)
...
예적금 중도해지 선택
  → 가상 고객 A
  → 상담 브리핑: 5개 필수 안내·2개 주의 표현·DEP-2026.08-v4
  → 로컬 replay 세션 생성
  → 3분 30초 replay 상태
  → 이벤트 순차 반영
  → 00:35 숫자 오류 개입
...
주택담보대출 선택
  → 가상 고객 B
  → 상담 브리핑: 5개 필수 안내·2개 주의 표현·MTG-2026.08-v1 데모 팩
  → 로컬 replay 세션 생성
  → 4분 replay 상태
  → LTV 고지·확정 전 금지·수수료 숫자 경보
  → 01:00 확정 전 안내 개입
  → 말씀 확정 vs 심사·결재 후 확정
  → 근저당 질의·필요 서류 assist
...
| trace | 로컬 이력 세션은 저장 이벤트 경로로 재생, 서버 이력은 events/report 조회 | 서버 세션을 대시보드 TRACE로 직접 재생하는 화면은 후속 작업 |
...
1. 사용자가 배포 URL에 접속한다.
2. 로그인 없이 랜딩을 본다.
3. 기본 상담 유형 `예적금 중도해지`와 기본 모드 `live`를 확인한다.
4. 필요하면 세션 설정을 열어 제품·모드·가상 고객 라벨을 바꾼다. 실제 연동에서는 고객 프로필 유형·태그도 함께 전달한다.
5. `상담 시작하기` 또는 `대시보드 열기`를 누른다.
6. 서버가 세션을 만들고 선택된 pack version을 고정한다.
7. `ready`와 함께 체크리스트 초기 상태를 받고, LIVE라면 사용자가 대시보드의 `녹음 시작` 버튼을 눌러 브라우저 녹음을 켠다. 같은 버튼으로 끄며, STT가 미설정이어도 세션은 LIVE로 유지한다.
8. 상담 전 브리핑 카드에 필수 항목 수, 최근 규정 변경, 모드를 표시한다.
9. 사용자가 상담 라이브로 진입한다.
...
| 상황 | 사용자에게 보여 줄 처리 |
| --- | --- |
| 상담 유형 미선택 | `상담 유형을 선택해 주세요` |
| 팩 없음 | 팩 없음 안내 + 가능한 제품 선택 |
...
- 주택담보대출 5개 체크리스트와 항목별 근거
- 근저당 질의의 근거 연결 답변, 필요 서류 assist
- 종료 리포트·이력·TRACE 재생

#### 주의할 현재 범위

- 주담대 `MTG-2026.08-v1`은 실제 서버 fixture가 아닌 프론트 데모 팩으로 명시한다.
- 실제 계약 연동 시에는 이 값을 확정 데이터로 사용하지 않고 서버의 `ready.pack_version`을 사용한다.

## 10. 기획상 화면·기능 전체 지도

| 화면 | 목표 역할 | 현재 상태 |
| --- | --- | --- |
| L0 랜딩 | 제품 설명, 세션 설정, 상담 시작 | 랜딩 UI와 로컬 전환 구현 |
...
| 2층 | 상담 브리핑 | 시작 전 준비 | 필수 안내·주의 표현·최근 변경 카드 |
...
| `GET /api/presets` | 랜딩 기본 프리셋·시연 선택 |
...
| 항목 | 현재 | 목표·문제 |
| --- | --- | --- |
| 모드 | TEXT 판정은 서버 WebSocket으로 연결, LIVE는 STT 미설정 시 TEXT 폴백, REPLAY/TRACE는 로컬 시나리오 | STT·replay 오디오·서버 trace 스트림은 후속 연결 대상 |
| 데이터 | `ready` 필수 항목·pack version, 서버 이벤트·progress·report를 반영 | 서버 outbound seq 기반 완전한 resume은 후속 검증 대상 |
| 제품 팩 | 발행 fixture 목록·팩 상세·브리핑을 서버 API 우선 조회 | 주택담보대출 발행 팩과 팩 발행 액션은 후속 연결 대상 |
...
- 랜딩은 첫 화면에서 서비스 컨셉을 강하게 제시하고, 아래의 가치·작동 방식·워크스페이스 설명으로 설득을 이어간다. `상담 시작하기`/`대시보드 열기`는 브리핑 진입용 주 CTA로 두고, 녹음·분석은 대시보드의 책임으로 분리한다. 세션 설정·보조 메뉴는 접거나 낮은 강도로 둔다.
- 브리핑은 `상담 화면으로 이동`을 주 버튼으로 두고 `설정으로 돌아가기`는 텍스트 버튼으로 둔다.
...
2. 랜딩의 제품·모드·고객 유형을 `POST /sessions`와 `hello`에 전달한다.
...
1. 페이지 PNG·팩 발행·replay 오디오 API를 백엔드와 함께 연결한다.
2. 서버 세션의 이벤트를 대시보드 TRACE 화면으로 직접 재생하고, 오디오 타임라인 클릭 이동을 추가한다.
...
- [x] 랜딩 선택값이 세션 생성·`hello`·대시보드에 이어진다.
- [x] 기본 경로는 `예적금 중도해지 · live`다. 브리핑 이후 대시보드에서 `녹음 시작` 버튼으로 on/off하며, STT 미설정 시에도 녹음 토글은 유지한다.
...
- [ ] 서버 이력에서 세션을 대시보드 TRACE로 직접 재생한다. 현재는 서버 세션 리포트 조회와 로컬 TRACE 재생까지 구현했다.
...
| TEXT·고객 유형 | TEXT 선택, 전문금융소비자 전달 UI, 상담 진입·발화 입력 경로 확인 |
