...
export function HistoryScreen({ onOpen, onStartPreset, busy, error, initialView = 'sessions', packVersion = '', ...navigation }: Navigation & { onOpen: (record: ApiSessionSummary, action: HistoryAction) => void; onStartPreset: (preset: ApiPreset) => void; busy: boolean; error: string; initialView?: 'sessions' | 'presets'; packVersion?: string }) {
  ...
  return <Workbench screen="history" title="세션 이력" subtitle="저장된 상담과 시연 음원" {...navigation} actions={busy ? <span className="wb-badge" role="status">상담 확인 중…</span> : <button onClick={view === 'sessions' ? records.refresh : refreshPresets} disabled={view === 'sessions' ? records.loading : presets.loading || packCatalog.loading}>새로고침</button>}>
    ...
    <div className="wb-toolbar wb-history-toolbar"><Tabs value={view} onChange={setView} items={[{ value: 'sessions', label: '저장된 상담' }, { value: 'presets', label: '시연 음원' }]} />{view === 'sessions' ? <label>입력 방식<select aria-label="이력 입력 방식" value={mode} onChange={event => setMode(event.target.value)}><option value="">전체 (재생 기록 제외)</option>{(['live', 'text', 'replay', 'trace'] as const).map(value => <option key={value} value={value}>{modeNames[value]}</option>)}</select></label> : <small className="wb-history-note">{packVersion ? `고른 규정팩 ${packVersion} 의 음원이 위에 옵니다. 다른 팩 음원은 그 팩 기준으로 판정됩니다.` : '음원마다 상품 시나리오와 규정팩이 정해져 있습니다.'}</small>}</div>
    {view === 'presets' ? <Panel className="wb-history-list"><PagedList label="시연 음원" items={playablePresets} rowHeight={100} empty={presets.loading || packCatalog.loading ? '시연 음원을 불러오는 중입니다.' : presets.error || packCatalog.error ? '음원을 확인하지 못했습니다.' : presets.settled && packCatalog.settled ? '최신 규정팩에 연결된 시연 음원이 없습니다.' : ''} render={preset => {
