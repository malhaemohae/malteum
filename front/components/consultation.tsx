...
export function Dashboard({ session, pack, health, micActive, micPending, micError, replaySound, onReplaySound, onMic, onEnd, onRetry, onTextMode, onNavigate, onNew, onCommand, onDismiss, onEvidence, onItemEvidence, onAsk }: DashboardProps) {
  ...
  function requestRephrase(code?: string) { if (onCommand({ t: 'assist_request', assist_type: 'rephrase', ...(code ? { item_code: code } : {}) })) { setSelected(null); setRephraseOpen(true); } }
  const rephraseItem = session.action?.kind === 'rephrase' && session.action.itemCode ? pack?.items.find(entry => entry.code === session.action?.itemCode) : undefined;
  const rephraseText = session.action?.result?.text ?? rephraseItem?.plain_language?.join('\n');
  ...
  return <Workbench screen={screen} title={title} subtitle={pack?.product?.name ?? session.packVersion} onNavigate={onNavigate} onNew={onNew} actions={<><span className="wb-badge">{session.status === 'connected' ? modeNames[session.mode] : session.status === 'connecting' ? '연결 중' : session.status === 'ended' ? '종료' : '연결 끊김'}</span><small>{session.mode === 'text' ? `${session.transcript.length}개 발화` : timeLabel(session.seconds)}</small><button disabled={session.status !== 'connected' || session.ending} onClick={onEnd}>{session.ending ? '종료 확인 중…' : playback ? '재생 종료' : '상담 종료'}</button></>}>
    ...
    {rephraseOpen && <Modal title={rephraseItem ? `쉬운 말 · ${rephraseItem.name}` : '직전 발화 쉬운 말'} onClose={() => setRephraseOpen(false)} actions={<>{(session.action?.result?.evidenceRef || rephraseItem?.evidence) && <button onClick={() => session.action?.result?.evidenceRef ? showEvidence(session.action.result.evidenceRef) : rephraseItem && showItemEvidence(rephraseItem)}>근거 원문</button>}<button disabled={!canWrite || manualPending} onClick={() => requestRephrase(session.action?.itemCode)}>{manualPending ? '기록 중…' : '다시 요청'}</button></>}>
