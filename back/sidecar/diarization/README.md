...
| 보낸다 | 청크(0.96초)마다 `{"segments": [{"start_ms", "end_ms", "speaker_id"}, ...], "covered_ms": 사이드카가 실제로 처리한 지점(ms)}` |
| 상태 확인 | `GET /health` |

돌려주는 것은 **지금까지의 구간 목록 전체**다. Sortformer 는 뒤 오디오를 보고 앞 구간을
고쳐 잡으므로 받는 쪽은 목록을 통째로 갈아 끼워야 한다.

`covered_ms` 는 이 목록이 **어디까지의 오디오를 보고** 나온 값인지를 알려 준다. 사이드카는
청크 하나가 찬 뒤에야 구간을 늘려 주므로 보낸 오디오 길이와 1초 남짓 어긋나는데, 발화 단위
전사 어댑터는 그 어긋남만큼을 무음으로 오해해 화자가 말하는 중에 발화를 닫아 버린다. 그래서
구간이 닫혔는지는 보낸 길이가 아니라 이 값으로 잰다(`server/services/stt/openai_file.py`).
