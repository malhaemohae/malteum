...
- **REST 는 계약이 정의한 상태 코드 밖으로 나가지 않는다.** `tests/server/test_contract_status_codes.py` 가 전 경로를 계약과 대조한다. 나가야만 하는 자리는 그 파일의 `BEYOND_CONTRACT` 에 이유를 적어야 통과하는데, **지금 비어 있고 그게 정상이다.** 무언가 적어야 하는 상황이면 계약을 먼저 본다 — 계약에 `NotFound`·`ValidationFailed`·`Unauthorized` 응답 컴포넌트가 있으므로 `$ref` 한 줄이면 된다.
