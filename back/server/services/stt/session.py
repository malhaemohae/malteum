...
## 확정될 때까지 3초까지 붙잡는다 (DEC-7)

새 화자 번호의 발화는 역할이 확정될 때까지 `hold_ms`(기본 3초)만 붙잡았다가 내보낸다.
...
# 오디오 프레임이 이만큼 안 오면 마지막 구간을 닫는다.
#
# 프레임 하나가 담는 소리가 100ms 일 뿐, 100ms 마다 하나씩 오는 것이 아니다. 브라우저는
# Web Audio 버퍼 콜백이 찰 때마다 여러 개를 몰아 보내므로(`front/lib/audio.ts` 의
# `createScriptProcessor(4096, ...)`) 완벽한 회선에서도 묶음 사이가 250ms 남짓 벌어진다.
# 1.5초는 그 여섯 배쯤이라 회선이 잠깐 튀는 것과 녹음을 멈춘 것이 갈린다. 이 값은 아직
# 산술이고 실측이 아니다. 실제 상담의 간격 분포는 `ws/pacing.py` 가 로그로 남긴다
IDLE_FLUSH_MS = 1500
