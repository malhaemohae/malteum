...
- 창구 상담의 당사자는 보통 은행원 한 명과 고객 한 명이다. 이미 한 역할만 확정되어
  있으면 새 번호는 반대 역할일 가능성이 높다. 다만 말의 내용(누가 설명·안내하고 누가
  사정을 말하고 묻는지)으로 판단하고, 내용이 확정된 번호와 같은 쪽을 가리키면 화자
  분리가 한 사람을 두 번호로 가른 것이니 같은 역할로 답한다. 번호가 셋이라고 사람이
  셋인 것은 아니다.
- 짧은 인사말이나 맞장구("안녕하세요.", "네.", "감사합니다.") 만으로는 역할을 정할 수
  없다. 은행원도 고객도 똑같이 하는 말이다. 그런 발화뿐이면 어느 쪽을 고르든
  confidence 를 0.4 이하로 낸다 — 발화가 더 모인 뒤에 다시 묻는다.
