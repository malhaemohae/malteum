...
같은 문장이라도 화자가 다르면 cache_key 도 달라진다. 판정 정책(PROMPT_VERSION)이 바뀌어도 달라진다.
"""
...
from engine.tiers.l3.tools import PROMPT_VERSION
...
def build(
    ...
    others = tuple(
        ...
        if it.type in ("required", "forbidden") and it.code not in candidate_codes
    )
