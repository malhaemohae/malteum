...
- 대상: `LOAN-DOC-001`(자필 기재 문구). `LOAN-RDR-001` 은 2026-09-03 에 p.3 정의 문장으로 근거를 바꿔 해제됨(`STATUS.md`)
- 문제: 문자열 진위는 통과하지만 필요 서류를 그 문구만으로 입증하지 못함. 가계대출 설명서에는 구비서류 목록이 없음
- 현재 처리: `evidence_scope_mismatch` 로 검토 대기
- 필요한 것: 신용대출 구비서류를 적은 다른 원천(은행 구비서류 안내 페이지 등)을 MANIFEST 에 추가하고 항목 근거를 그쪽으로
