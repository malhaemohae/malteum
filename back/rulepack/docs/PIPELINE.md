...
2026-09-03 baseline (e5-small 단독 · top-k 3 · 발행 가능 전 항목, 예금·대출 모두 `contracts/fixtures/` 의 v4 팩. 2026-09-02 측정치는 대출 8항목 synthetic 팩 기준 6/8 · 7/8 이었고, 금지 예시 보강·숫자 사실 추가·골든셋 1건 추가 뒤 다시 잰 값이 아래):

| 팩 | top-1 | recall@3 | 관련 top1 대역 | 무관 top1 대역 |
| --- | ---: | ---: | --- | --- |
| 예금 9항목 | 7/10 | 8/10 | 0.846~0.944 | 0.806~0.822 |
| 대출 8항목 | 5/9 | 8/9 | 0.845~0.954 | 0.793~0.815 |

recall@3 실패는 `DEP-PRO-001`(6위) · `DEP-LIM-001`(4위) · `LOAN-WDR-001`(6위) 셋. 대출 top-1 이 낮은 것은 단정 발화 패러프레이즈(`loan-ban1-fixed-rate-assertion`)가 `LOAN-RSK-001` 에 1위를 내주고 DSR 질문이 `LOAN-BAN-001` 뒤에 서기 때문이며, 둘 다 recall 은 통과. 관련 최저와 무관 최고의 간격이 0.02 안팎이라 절대 점수 임계값 게이트는 아직 못 세운다. 엔진의 자모 trigram 융합 같은 검색 방식 변경은 이 표와 같은 조건으로 다시 재서 대조한다.

재현 조건 세 가지를 지켜야 이 표와 대조가 된다.

- **팩 경로를 명시한다.** 예금은 `contracts/fixtures/rulepack_DEP-2026.08-v4.json`, 대출은 `contracts/fixtures/rulepack_LOAN-2026.08-v4.json`. 인자 없는 기본 실행은 `rulepack/artifacts/` 의 로컬 팩을 집는데, 그 폴더에는 옛 3항목짜리 검증용 팩이 남아 있을 수 있어 수치가 통째로 어긋난다
