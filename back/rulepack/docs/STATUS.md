...
기준일: 2026-09-03. `python -m rulepack.cli build` 결과와 `config/source_audit.json` 에서 뽑은 값이다.
...
- 2026-09-03 숫자 사실(`numeric_facts`)마다 자기 근거를 둘 수 있게 파이프라인 수정(`pipeline._verify_numeric_fact_evidence`). 계약 스키마는 원래 사실별 `evidence` 를 허용했는데 M3 후보 설정 스키마가 그 키를 금지하고 생성 단계가 항목 근거로 덮어쓰고 있었음. 자기 근거는 원문 대조(exact span 유일 + bbox)만 거치고 chunk 지원 검사는 거치지 않음. chunk 검사는 LLM 이 본 덩어리에서 나온 후보인지를 보는 장치라 사람이 설정에 적은 숫자에는 해당이 없고, 표 안의 표처럼 구조 추출이 잃는 줄(정기예금 p.2 중도해지이율 표)도 pdfium 텍스트에는 살아 있어 이 경로로만 근거를 댈 수 있음. `tests/rulepack/test_numeric_facts.py` 가 상시 검사
- 2026-09-03 `LOAN-ARR-002` 되돌림. 위 수정으로 연체가산이자율 연 3% 를 `LOAN-ARR-001` 의 숫자 사실(p.7 자기 근거)로 실을 수 있게 되어 항목을 쪼갤 이유가 사라짐. `LOAN-WDR-001` 에 청약철회 14일(p.3 자기 근거) 추가. 발행 가능 18 → 17건
- 2026-09-03 `DEP-TAX-001` 에 세율 15.4% 를 숫자 사실로(p.1 자기 근거). 예금 팩의 ⑤ 숫자 대조는 이 항목 하나가 절대값을 가진 유일한 자리. `DEP-INT-002` 의 옛 원천 0.10% 는 fixture 에서 제거(아래 재발행 참조)
- 2026-09-03 계약 fixture 를 canonical 에서 다시 발행해 동기화. 예금 `DEP-2026.08-v4` 는 같은 버전으로 제자리 재발행(M1 `default_pack_version`·M1/M2 테스트가 파일명을 고정하고 있어 버전을 올리지 않음. 옛 원천 0.10% 부채가 사라져 canonical 과 드리프트 0). 대출은 `LOAN-2026.08-v4` 로 올리고 v3 파일은 제거. 시나리오 A 이벤트 fixture 는 손으로 고치지 않고 대본(`assets/scenarios/preset-dep-a/script.json`)에서 `scripts/gen_scenario_trace.py` 로 생성(48건). 판정 케이스 16건·WS 메시지·M2 테스트 데이터도 같은 대사로 맞춤. **배포 DB 동기화 필요**: `load_pack --replace DEP-2026.08-v4` 와 `load_pack LOAN-2026.08-v4`, `pack_version` 을 키로 쓰는 L3 판정 캐시 무효화
- 2026-09-03 중도해지이율·만기후이자율처럼 `약정이율×0.5` 꼴 비율·산식은 숫자 사실을 두지 않기로 함(팀 결정). `value + unit` 절대값 모델로는 "약정이율의 절반" 을 표현할 수 없고, 억지로 50% 를 넣으면 은행원이 0.5% 라고 말했을 때 "설명서 기준 50%" 라는 오해를 부르는 경보가 남. 산식을 말했는지는 L1 정규식(`차감률|산출식|절반|0.5|50%|곱해` 등)이 잡고, 내용이 맞는지는 L3(LLM) 가 근거 문장을 보고 판단
