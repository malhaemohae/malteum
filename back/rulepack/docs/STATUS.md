...
- 발행 가능 18건: `DEP-INT-001` · `DEP-INT-002` · `DEP-INT-003` · `DEP-PRO-001` · `DEP-TAX-001` · `DEP-LIM-001` · `DEP-DOC-001` · `DEP-BAN-001` · `DEP-RSK-001` · `LOAN-INT-001` · `LOAN-RDR-001` · `LOAN-ARR-001` · `LOAN-PRE-001` · `LOAN-DSR-001` · `LOAN-WDR-001` · `LOAN-CIC-001` · `LOAN-BAN-001` · `LOAN-RSK-001`
...
### 검토 대기 1건 · 근거의 의미 범위 부족

`LOAN-DOC-001`(필요 서류)은 exact span("자필로 기재" 안내)은 원문에 실재하지만, 그 문구만으로 신용대출 구비서류를 입증하지 못한다. 가계대출 설명서에는 구비서류 목록 자체가 없어 다른 원천(은행 구비서류 안내)이 필요하다. `evidence_scope_mismatch` 로 사람 검토를 기다린다. `docs/CONTRACT_GAPS.md` 참조.
