...
('1. 서비스 명칭',
 ...
 '<b>검증 현황</b>: 판정 규칙 계약 테스트 17건과 백엔드 테스트 512건이 서버 없이 통과. 시연 음원 2편 실측으로 전사 오류율 3.6% · 0.4%, 화자 확정 평균 0.61초, 재판정 왕복 평균 1.64초')
...
('4. 서비스 컨셉 및 차별성',
 ...
 ('배치 전제','클라우드 API 호출을 전제로 설계','은행 내부망에는 인터넷이 없다는 전제로 추론 어댑터를 분리. 시연은 공개 URL, 제품 배치는 내부망 모델')],[18,39,43])
...
('5. 활용 데이터 및 생성형 AI 모델 적용 방안',
 ...
 ('규정·상품 문서','공개 공식 문서 10종을 해시로 고정해 보관. 현재 두 팩이 실제로 인용 좌표를 대는 것은 금융소비자보호법, 예금거래기본약관, 상품설명서 2종, 상품 공시, 보이스피싱 예방 10계명 6종','항목의 기준과 원문 인용 위치 제공'),
 ...
 + bullets('<b>STT(Speech To Text: 음성의 문자 변환)</b>: OpenAI Realtime 으로 음성을 발화로 변환. Sortformer 화자 분리와 역할 추정으로 직원 설명과 고객 반응을 구분. 시연은 외부 추론 서비스를 쓰고 제품 배치는 내부망 모델로, 어댑터만 교체되도록 분리',
 '<b>검색</b>: 어휘·자모 유사도와 임베딩을 이용해 관련 규정 후보를 좁힘. 유사도 자체를 설명 완료로 해석하지 않음',
 '<b>LLM(Large Language Model: 대규모 언어 모델) ① 상담 중 재판정</b>: Qwen3-8B 로 규칙이 가르지 못한 설명의 취지를 제한된 후보 안에서 판정. 허용 항목·상태 변경·결과 형식을 검사한 뒤에만 반영. 재판정 15건 왕복이 최소 1.24 · 평균 1.64 · 최대 2.06초로 3초 예산 초과 0건',
 '<b>LLM ② 룰팩 구축(상담 전 오프라인)</b>: 공식 문서에서 항목·요건·쉬운 말·수치 후보를 구조화 추출. 생성된 후보 중 원문 구절·페이지·좌표 대조에 실패한 것은 자동 폐기하고, 통과한 것만 사람 검수·서명을 거쳐 발행. 정기예금 12항목·신용대출 15항목이 이 관문을 통과한 결과',
...
('7. 기술 검증 실측과 도입 전략',
 ...
 ('자체 서버로 옮길 수 있는가','온프레미스(내부망 자체 서버) 후보를 자르지 않고 연속으로 흘려 봄',
  '20~30초를 넘기면 쓴 문장을 다시 쓰거나 없는 문장을 끼워 오류율이 31%에서 385%까지 무너짐(글자 오류율은 끼워 넣기가 쌓이면 100%를 넘음. 385% 는 전사 길이가 정답의 4~5배). 화자 구간으로 끊는 것을 전환의 전제 조건으로 확정'),
 ('화자 구분을 실시간에 쓸 수 있는가','화자 분리 모델의 청크(오디오를 자르는 단위) 길이를 0.96초부터 15초까지 4단계로 바꿔 측정',
  '0.96초 청크에서 화자가 정해지기까지 평균 0.61초 · 최대 1.09초. 두 음원 32줄 전건 정확'),
 ('LLM 재판정이 상담 속도를 해치는가','시연 음원 2편을 서버로 4회 재생하고 재판정 15건의 왕복 시간을 측정',
  '최소 1.24 · 평균 1.64 · 최대 2.06초. 3초 예산을 넘긴 건 0건이고 종료 요약이 기대값과 일치'),
 ('전사가 틀려도 규칙 판정이 살아남는가','음성 인식이 잘못 받아 적은 14벌로 규칙 매칭을 다시 돌림',
  '기대 매칭 154개 중 109개에서 135개로. 영문 약어 교정과 공백 제거 매칭을 넣어 개선'),
 ],[21,37,42])
 + h('이 실측이 설계로 이어진 지점')
 ...
 '<b>수치는 모델이 고치지 않음</b>: 전사 교정을 허용 후보 선택으로 제한해 잘못 말한 값이 정답으로 바뀌는 경로를 없앰')
 + source('원자료와 재현 명령은 저장소 docs/실험/ 에 있음. 팀이 만든 시연 음원과 개발 환경에서 잰 값이며 실제 창구의 소음·다화자 조건을 보장하지 않음.')),
('7. 기술 검증 실측과 도입 전략',
 h('단계별 통과 기준')
...
('1. MVP 구현 범위',
 ...
 + source('2026-09-06 저장소 계약 fixture 의 항목 배열을 직접 집계한 값. 전체 금융 규정의 포괄률을 의미하지 않음.')
 + bullets('새 상담은 선택한 팩 버전으로 유지하며, 과거 기록은 당시 적용한 버전을 기준으로 조회',
 '판정 규칙은 입력·기대 출력을 고정한 계약 테스트 17건으로 검증함. 백엔드 테스트 512건이 함께 돌고 CI 가 자동 실행함',
 '실서버 모델 연결 상태와 확인 절차는 5번에 별도 기재')
),
('2. 주요 기능 목록',
 source('상태 구분: 계약 테스트 = 입력·기대 출력을 고정한 시험이 통과 · 화면 확인 = 로컬 브라우저에서 조작까지 확인 · 모델 연결 시 = STT·LLM·임베딩이 붙어야 동작 · 배포 확인 필요 = 홈서버에서 다시 확인')
 + table(['기능명','기능 설명','관련 화면','구현 상태'],[
 ('상담 준비','최신 상품 팩과 필수 설명을 확인하고 상담 생성','상담 준비','화면 확인'),
 ('음성 상담','마이크 음성을 서버에 전달하고 확정 발화를 수신','상담','화면 확인 · 배포 확인 필요'),
 ('텍스트 시험','화자를 구분한 텍스트 입력으로 판정 경로 확인','상담 · 텍스트 입력','계약 테스트 17건 · 화면 확인'),
 ('필수 안내 추적','설명 항목과 빠진 요건을 구분해 표시','필수 안내','계약 테스트 L1 9건 · L3 4건'),
 ('경보 확인','수치 불일치·금지 표현·위험 신호. 규칙 판정이라 모델 없이도 동작','상담 가이드 · 현재 안내','계약 테스트 · 오탐 회피 8건 포함'),
 ('쉬운 말 안내','승인 문장 조회와 직전 발화 재설명 요청을 구분','쉬운 말·상담 가이드','화면 확인'),
 ('규정 질의','팩에서 관련 항목과 연결된 근거 제시','상담 가이드 하단 질문창','계약 테스트 · 모델 연결 시'),
 ('근거 원문 탐색','형광펜 위치와 문서의 주변 문맥·페이지 확인','근거 원문','화면 확인'),
 ('직원 수동 조치','설명 완료·해당 없음·경보 확인 등 조치 기록','필수 안내·경보','계약 테스트 · 화면 확인'),
 ('이력·기록 재생','종료 상담과 저장된 이벤트를 순서대로 조회','세션 이력','화면 확인'),
 ('리포트·PDF','항목별 결과와 근거·타임라인 조회 및 저장','종료 리포트','화면 확인'),
 ('규정·문서 조회','최신 팩 구성과 보관된 원문 문서를 조회','규정 관리','화면 확인')],[19,44,18,19])
...
('4. AI 및 데이터 처리 방식',
 ...
 ('L0 정규화','발화자·신뢰도 확인과 팩 용어 사전 치환','판단할 범위를 제한하고 원문은 그대로 보존'),
...
('5. MVP 검증 방법',
 h('서버 없이도 확인되는 판정 계약 테스트')
 + source('입력 발화와 기대 출력을 쌍으로 고정한 표임. 구현이 바뀌어도 이 표가 통과해야 함. 묶음은 어느 층이 판정하는지, 상한은 그 층이 넘으면 안 되는 시간임.')
 ...
 ('사람 결정','1건','없음','면제는 사람만 낼 수 있고 사유가 없으면 거절')],[18,12,16,54])
 + h('17건 중 8건이 잘못 잡지 않는 것을 확인')
 + table(['시험','확인하는 것'],[
 ...
 ('근거 없는 질문 → 답하지 않음','팩에 없는 것을 물으면 빈 값을 돌려주고 문장을 만들지 않음'),
 ('산식 설명 → 숫자 대조 대상 아님','약정이율의 절반처럼 비율로 정해지는 값은 절대값 대조를 걸지 않음'),
 ('무관한 발화 → 아무 이벤트 없음','상담과 관계없는 말에는 판정도 경보도 만들지 않음'),
 ('면제는 사람만 · 사유 없으면 거절','엔진이 스스로 면제를 만들 수 없고 사유가 비면 받지 않음')],[34,66])
 + note('실행 명령은 저장소에서 make test 임. 이 표는 판정 규칙의 계약이며 음성 인식 정확도를 재는 시험이 아님. 실제 모델 왕복을 감안한 실행 예산은 3초로 따로 두었음.')),
('5. MVP 검증 방법',
 h('접속 준비')
 ...
 '<b>저장소</b>: 공개 저장소에서 compose 한 줄로 로컬 실행 [배포 후 주소 기입]',
 '<b>계약 테스트</b>: 저장소에서 make test 실행. 판정 규칙 17건과 백엔드 512건이 서버 없이 통과')
 + h('실행 환경')
...
('5. MVP 검증 방법',
 ...
 ('A6','종료 리포트에서 PDF 저장','파일을 다시 열어 팩 버전·항목별 기록 확인'),
 ('A7','텍스트 입력으로 아래 B 의 숫자·금지·위험 발화 입력','STT·LLM 이 꺼져 있어도 세 경보가 규칙만으로 표시됨')],[10,45,45])
...
('5. MVP 검증 방법',
 ...
 ('위험 신호','DEP-2026.08-v6 · 고객 발화: “해지한 돈은 딸이 알려준 계좌로 바로 보내 주세요.”','DEP-RSK-001 의 위험 신호 경보가 최우선으로 표시되고 확인 기록을 남길 수 있음. 규칙 판정이라 모델 없이도 동작'),
 ('숫자 오류','DEP-2026.08-v6 · 텍스트 입력 · 직원 발화: “받으시는 이자에는 과세가 되는데요, 세율은 14%입니다.”','DEP-TAX-001에서 발화 14%와 팩 기준 정상일반세율 15.4%의 차이를 짚는 경보 및 원문 근거 확인'),
 ('금지 취지 · 규칙','LOAN-2026.08-v7 · 직원 발화: “대출은 무조건 승인됩니다.”','LOAN-BAN-001의 금지 표현 경보가 규칙만으로 즉시 표시됨. 모델 연결과 무관'),
 ('금지 취지 · 의미','직원 발화: “대출 승인은 무조건 확정입니다.”','규칙이 놓치는 우회 표현. 의미 재판정이 연결됐을 때만 경보. 후보만 표시되면 최종 판정 성공으로 세지 않음'),
 ('규정 질문','신용대출 팩에서 “중도상환수수료는 얼마인가요?” 질문','임베딩 연결 시 LOAN-PRE-001 의 승인 문장과 출처 페이지 표시. 필요 서류는 질의가 아니라 상담 준비의 필요 서류 버튼이 담당'),
 ...
 + source('교체 화면 S1(배포 후 삽입): 정기예금 세율 오류 장면을 실제 추론으로 만든 뒤 ① 발화 ② 발화값과 기준값 ③ 원문 근거를 빨간 박스로 표시. 세션 번호·입력 방식·팩 버전·시각을 촬영 기록에 남김.')
...
('5. MVP 검증 방법',
 ...
 + source('교체 대상: 실서버에서 새로 만든 발화·경보 화면과 같은 세션의 종료 리포트, 저장 PDF. 접속 URL 과 최종 모델 구성도 배포 후 기입.')),
]
