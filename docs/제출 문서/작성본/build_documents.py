...
('4. 서비스 컨셉 및 차별성',
 figure('05-userflow','상담원이 근거를 확인하고 설명을 보완하는 흐름. 경보에서 근거로 내려갔다가 상담으로 돌아오는 갈래가 한 상담에서 여러 번 일어남.',True)
 + h('핵심 사용 장면')
...
('7. 기술 검증 실측과 도입 전략',
 ...
 ('자체 서버로 옮길 수 있는가','온프레미스(내부망 자체 서버) 후보를 자르지 않고 연속으로 흘려 봄',
  '20~30초를 넘기면 쓴 문장을 다시 쓰거나 없는 문장을 끼워 오류율이 31%에서 385%까지 무너짐. 화자 구간으로 끊는 것을 전환의 전제 조건으로 확정'),
...
('1. MVP 구현 범위',
 ...
 '판정 규칙은 입력·기대 출력을 고정한 계약 테스트 17건으로 검증함. 백엔드 테스트 326건이 함께 돌고 CI 가 자동 실행함',
...
('2. 주요 기능 목록',
 ...
 ('경보 확인','수치 불일치·금지 표현·위험 신호. 규칙 판정이라 모델 없이도 동작','상담 가이드 · 현재 안내','계약 테스트 · 오탐 회피 5건 포함'),
...
('5. MVP 검증 방법',
 ...
 ('근거 없는 질문 → 답하지 않음','팩에 없는 것을 물으면 빈 값을 돌려주고 문장을 만들지 않음')],[34,66])
...
('5. MVP 검증 방법',
 ...
 '<b>계약 테스트</b>: 저장소에서 make test 실행. 판정 규칙 17건과 백엔드 326건이 서버 없이 통과')
 + h('실행 환경')
