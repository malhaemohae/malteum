...
# ---------------------------------------------------------------------------------
def d01_system() -> Svg:
    s = Svg("01-system", "말틈 전체 구성", "상담원 브라우저가 홈서버의 프론트엔드·게이트웨이·내장 엔진·저장소와 연결되고, 음성 인식과 언어 모델은 설정에 따라 외부 추론으로 붙는 구성", 1000, 448)
    s.dy = -96
    s.zone(196, 196, 600, 244, "HOME SERVER · DOCKER COMPOSE")
    s.zone(800, 136, 176, 272, "EXTERNAL INFERENCE · 설정 의존")
    # arrows first
    s.line(168, 264, 216, 264, "link"); s.label(192, 244, "HTTPS", LINK)
    s.line(344, 264, 400, 264); s.label(372, 244, "WS·REST")
    s.line(480, 296, 480, 344); s.label(512, 312, "JUDGE")
    s.line(560, 248, 616, 248); s.label(588, 228, "APPEND")
    s.line(616, 280, 560, 280, dashed=True); s.label(588, 288, "RESTORE")
    s.line(344, 376, 400, 376); s.label(372, 356, "LOAD")
    s.arrow("M 480,232 V 184 Q 480,176 488,176 H 816"); s.label(700, 156, "AUDIO 16kHz PCM")
    s.arrow("M 816,208 H 528 Q 520,208 520,216 V 232", dashed=True); s.label(700, 216, "TRANSCRIPT·SPEAKER")
    s.arrow("M 560,376 H 788 Q 796,376 796,368 V 360 Q 796,352 804,352 H 816"); s.label(688, 356, "L3 JUDGE · ASSIST")
    # nodes
    s.node(40, 232, 128, 64, "상담원 브라우저", "Chrome · Edge", "input", "USER")
    s.node(216, 232, 128, 64, "프론트엔드", "Next.js 14", "step", "WEB")
    s.node(400, 232, 160, 64, "게이트웨이", "FastAPI · WS + REST", "focal", "API")
    s.node(400, 344, 160, 64, "내장 엔진", "L1 규칙 · L2 검색 · L3 재판정", "step", "LIB")
    s.node(616, 232, 144, 64, "상담 이벤트", "PostgreSQL · append-only", "store", "DB")
    s.node(216, 344, 128, 64, "승인된 룰팩", "버전 고정 JSON", "store", "PACK")
    s.node(816, 160, 144, 64, "음성 인식·화자 분리", "STT · Sortformer", "external", "STT")
    s.node(816, 320, 144, 64, "언어 모델", "재판정 · 역할 추정", "external", "LLM")
    s.legend([("focal", "게이트웨이"), ("step", "서버 구성요소"), ("store", "저장소"), ("external", "외부 추론"), ("link", "HTTP"), ("default", "주요 흐름"), ("dashed", "반환 · 복구")])
    return s


def d02_engine() -> Svg:
    s = Svg("02-engine", "엔진 판정 흐름 · 빠른 판정과 늦게 오는 재판정", "확정 발화가 정규화, 규칙 판정, 검색을 거쳐 즉시 화면에 반영되고, 필요할 때만 LLM 재판정이 뒤따라 앞선 판단에 연결되는 흐름", 1000, 424)
    s.dy = -152
    # row 1 arrows
    ...
    s.line(888, 404, 888, 256, dashed=True); s.label(936, 320, "SUPERSEDE")
    # nodes
    s.node(40, 200, 128, 56, "확정 발화", "화자 · 신뢰도 · 시각", "input", "", shape="oval")
    s.node(216, 200, 120, 56, "L0 정규화", "팩 용어 사전 치환", "step", "L0")
    s.node(384, 200, 120, 56, "L1 규칙", "요건 · 표현 · 수치", "step", "L1")
    s.node(552, 200, 120, 56, "L2 검색", "자모 3-gram + 임베딩", "step", "L2")
    s.node(752, 200, 208, 56, "화면 반영", "판정 · 경보 · 조력 카드", "step", "OUT")
    s.diamond(612, 332, 96, 32, "재판정 필요?", "partial · suspected")
    s.node(536, 400, 152, 64, "L3 재판정", "LLM · 허용 후보만 · 3초 예산", "focal", "L3")
    s.node(816, 404, 144, 56, "결과 기록·연결", "앞선 판단을 supersede", "store", "LOG")
    s.legend([("focal", "LLM 판정"), ("step", "규칙·검색 단계"), ("store", "기록"), ("accent", "즉시 반영 경로"), ("default", "흐름"), ("dashed", "뒤늦게 도착하는 갱신")])
    return s


def d03_correction() -> Svg:
    s = Svg("03-correction", "전사 교정 · 원문을 보존하는 제한적 보완", "확정 발화가 팩 용어 규칙 보완을 거치고, 재판정 경로에서만 선택적으로 LLM 이 허용 후보 안에서 용어 조합을 고르며, 검사에 실패하면 현재 해석으로 진행하는 흐름", 1000, 400)
    s.dy = -152
    s.line(176, 228, 224, 228); s.label(200, 208, "L0")
    ...
    s.node(40, 200, 136, 56, "확정 은행원 발화", "STT 결과 그대로", "input", "", shape="oval")
    s.node(224, 200, 136, 56, "규칙 보완", "팩 용어 · 띄어쓰기", "step", "RULE")
    s.node(408, 200, 136, 56, "후보 선별", "모호한 어절 · 용어 짝", "step", "SCOPE")
    s.diamond(660, 228, 72, 32, "보조 교정 사용?", "기본 설정 OFF")
    s.node(588, 300, 144, 56, "교정 에이전트", "LLM · 허용 후보 조합 선택", "focal", "LLM")
    s.node(588, 384, 144, 64, "조합 재검사", "허용 후보인지 · 짝이 맞는지", "step", "CHECK")
    s.node(816, 200, 144, 56, "재판정 입력 (L3)", "판정용 해석", "step", "JUDGE")
...
def d04_rulepack() -> Svg:
    s = Svg("04-rulepack", "룰팩 구축 파이프라인 · 원문에서 승인된 기준까지", "공개 규정 문서가 구조 추출, 후보 생성, 근거 대조, 사람 검수를 거쳐 버전이 고정된 팩으로 발행되고, 상담 중에는 그 팩을 읽기 전용으로 쓰는 흐름", 1000, 480)
    s.dy = -64
    s.zone(212, 108, 768, 212, "OFFLINE · 규정 분석과 사람 검수")
    s.zone(404, 332, 380, 104, "RUNTIME · 상담 중 읽기 전용")
    s.line(184, 176, 232, 176); s.label(208, 156, "PDF")
    s.line(376, 176, 424, 176); s.label(400, 156, "CHUNK")
    s.line(568, 176, 616, 176); s.label(592, 156, "DRAFT")
    s.line(760, 176, 808, 176, "accent"); s.label(784, 156, "RC", ACCENT)
    s.line(688, 208, 688, 248, dashed=True)
    s.line(884, 208, 884, 352); s.label(920, 272, "PUBLISH")
    s.line(808, 384, 760, 384); s.label(784, 364, "LOAD")
    s.line(616, 384, 568, 384); s.label(592, 364, "READ")
    s.node(40, 144, 144, 64, "원천 문서 10종", "법령 · 약관 · 상품설명서", "input", "SRC")
    s.node(232, 144, 144, 64, "구조 추출", "OpenDataLoader · 표·제목", "step", "PARSE")
    s.node(424, 144, 144, 64, "후보 생성", "LLM 구조화 추출", "step", "DRAFT")
    s.node(616, 144, 144, 64, "근거 대조", "find_span · page · bbox", "focal", "P4")
    s.node(808, 144, 152, 64, "사람 검수·승인", "리뷰 큐 · HMAC 서명", "step", "REVIEW")
    s.node(616, 248, 144, 48, "자동 폐기", "근거 없는 항목", "optional", "")
    s.node(808, 352, 152, 64, "발행된 팩", "rulepack_v6.json · 불변", "store", "PACK")
    s.node(616, 352, 144, 64, "적재", "PostgreSQL · 임베딩", "store", "DB")
    s.node(424, 352, 144, 64, "상담 세션", "L1/L2/L3 판정 기준", "step", "RUN")
...
def d05_userflow() -> Svg:
    ...
    s.node(40, 152, 128, 56, "접속", "배포 URL · 로그인 없음", "input", "", shape="oval")
    s.node(216, 152, 144, 64, "상담 준비", "최신 룰팩 · 필수 안내 · 서류", "step", "01")
    s.node(408, 152, 176, 64, "상담 진행", "녹음 또는 텍스트 · 실시간 전사", "focal", "02")
    s.node(632, 152, 136, 64, "상담 종료", "종료 버튼 한 번", "step", "03")
    s.node(816, 152, 144, 64, "종료 리포트", "항목별 기록 · PDF 저장", "step", "04", shape="oval")
