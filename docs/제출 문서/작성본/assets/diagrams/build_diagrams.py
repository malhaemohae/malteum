...
def d01_system() -> Svg:
    ...
    s.node(216, 344, 128, 64, "승인된 규정팩", "버전 고정 파일", "store", "PACK")
...
def d02_engine() -> Svg:
    ...
    s.node(216, 200, 120, 56, "L0 다듬기", "규정팩 용어로 맞춤", "step", "L0")
...
def d03_correction() -> Svg:
    s = Svg("03-correction", "받아쓴 문장 교정 · 원문을 보존하는 제한적 보완", "받아쓴 문장이 규정팩 용어로 보완되고, 재판정 경로에서만 선택적으로 언어 모델이 허용 후보 안에서 용어 조합을 고르며, 검사에 실패하면 원래 해석으로 진행하는 흐름", 1000, 400)
    s.dy = -152
    ...
    s.node(224, 200, 136, 56, "규칙 보완", "규정팩 용어 · 띄어쓰기", "step", "RULE")
...
def d04_rulepack() -> Svg:
    s = Svg("04-rulepack", "규정팩 구축 · 원문에서 승인된 기준까지", "공개 규정 문서가 구조 추출, 후보 생성, 근거 대조, 사람 검수를 거쳐 버전이 고정된 규정팩으로 발행되고, 상담 중에는 그 규정팩을 읽기만 하는 흐름", 1000, 480)
    s.dy = -64
    ...
    s.node(808, 352, 152, 64, "발행된 규정팩", "버전 고정 · 불변", "store", "PACK")
...
def d05_userflow() -> Svg:
    ...
    s.node(216, 152, 144, 64, "상담 준비", "규정팩 · 필수 안내 · 서류", "step", "01")
